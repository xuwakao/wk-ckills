...
3. PHASED DEVELOPMENT: Follow META-PHASE A→B→C→D. Each phase: execute → review → acceptance. Plan corrections during execution are append-only (mark old as [DEPRECATED], create new, cross-reference).
...
6. DOCUMENTATION: Update docs/progress/ and docs/issue/ continuously. Formal technical language only. All claims require verifiable sources. Mark unverified content [待验证]. Use /git-commit for commits.
